...
class Cell extends Border
{
     ...
     * @param $style
     */
    public function mergeBorderStyles($style): void
    {
        if (!$style) {
            return;
        }
        ...
        foreach (['Top', 'Left', 'Right', 'Bottom'] as $side) {
            foreach (['Size', 'Style', 'Color'] as $attribute) {
                $attributeName = "border$side$attribute";
                if (property_exists($this, $attributeName) && $this->$attributeName === null) {
                    $this->$attributeName = $style->$attributeName;
                }
            }
        }
    }
