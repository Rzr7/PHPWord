...
abstract class AbstractPart
{
    ...
    {
        if ($xmlReader->elementExists('w:r/w:t', $domNode)) {
            $nodes = $xmlReader->getElements('w:r/w:t', $domNode);
            foreach ($nodes as $node) {
                if (
                    strlen($node->textContent) > 5
                    && empty(trim($node->textContent))
                    && $xmlReader->getAttribute('xml:space', $node) == 'preserve'
                    && $xmlReader->getElements('*', $node)->length == 0) {
                    $parent->addTextBreak();
                }
            }
        }
        // Paragraph style
        $paragraphStyle = $this->phpWord ? $this->phpWord->getDefaultParagraphStyle() : null;
        $headingDepth = null;
        if ($xmlReader->elementExists('w:pPr', $domNode)) {
            /** @var Paragraph $localParagraphStyle */
            $localParagraphStyle = $this->readParagraphStyle($xmlReader, $domNode);

            if ($paragraphStyle === null) {
                $paragraphStyle = $localParagraphStyle;
                $this->getHeadingDepth($paragraphStyle);
            } else {
                $paragraphStyle = $paragraphStyle->getNewInstance();
                $paragraphStyle->mergeArray($localParagraphStyle);
                $headingDepth = $this->getHeadingDepth($paragraphStyle->getStyleValues());
            }
        }
    ...
    protected function readRunChild(XMLReader $xmlReader, \DOMElement $node, AbstractContainer $parent, $docPart, $paragraphStyle = null, $fontStyle = null)
    {
        ...
        if ($node->nodeName == 'w:footnoteReference') {
            ...
            $name = $xmlReader->getAttribute('name', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:nvPicPr/pic:cNvPr');
            $embedId = $xmlReader->getAttribute('r:embed', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:blipFill/a:blip');

            $target = $this->getMediaTarget($docPart, $embedId);
            if (!is_null($target)) {
                $imageSource = "zip://{$this->docFile}#{$target}";

                $noWrapMode = $xmlReader->elementExists('wp:*/wp:wrapNone', $node);
                $anchor = $xmlReader->elementExists('wp:anchor', $node);
                $inline = $xmlReader->elementExists('wp:inline', $node);

                $imageData = [
                    'posHorizontal' => $xmlReader->getValue('wp:*/wp:positionH/wp:align', $node),
                    'posVertical' => $xmlReader->getValue('wp:*/wp:positionV/wp:align', $node),
                    'top' => $xmlReader->getValue('wp:*/wp:positionV/wp:posOffset', $node),
                    'left' => $xmlReader->getValue('wp:*/wp:positionH/wp:posOffset', $node),
                    'borderWidth' => $xmlReader->getAttribute('w', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:spPr/a:ln'),
                    'borderColor' => $xmlReader->getAttribute('val', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:spPr/a:ln/a:solidFill/a:srgbClr'),
                    'width' => $xmlReader->getAttribute('cx', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:spPr/a:xfrm/a:ext'),
                    'height' => $xmlReader->getAttribute('cy', $node, 'wp:*/a:graphic/a:graphicData/pic:pic/pic:spPr/a:xfrm/a:ext'),
                ...
                if ($anchor) {
                    foreach (['simplePos', 'behindDoc', 'layoutInCell', 'allowOverlap', 'locked'] as $attributeName) {
                        $imageData[$attributeName] = (bool)(int)$xmlReader->getAttribute($attributeName, $node, 'wp:anchor');
                    }
                }

                $parent->addImage($imageSource, $imageData, true, $name);
            }
            ...
            if ($runParent->nodeName == 'w:r' && $node->nodeName == 'w:t') {
                $parent->addTextBreak();
            }
    ...
    protected function readTable(XMLReader $xmlReader, \DOMElement $domNode, $parent, $docPart = 'document')
    {
        ...
        foreach ($tblNodes as $tblNode) {
            if ('w:tblGrid' == $tblNode->nodeName) { // Column
                foreach ($xmlReader->getElements('w:gridCol', $tblNode) as $gridCol) {
                    $table->columnWidths[] = $xmlReader->getAttribute('w:w', $gridCol);
                }
                ...
                foreach ($rowNodes as $rowNode) {
                    if ('w:trPr' == $rowNode->nodeName) { // Row style
                        ...
                        foreach ($cellNodes as $cellNode) {
                            ...
                            } elseif ('w:tbl' == $cellNode->nodeName) { // Paragraph
    ...
    protected function readParagraphStyle(XMLReader $xmlReader, \DOMElement $domNode)
    {
        ...
        $styleDefs = [
            ...
            'lineHeight'          => [self::READ_VALUE, 'w:spacing', 'w:line'],
            'lineRule'            => [self::READ_VALUE, 'w:spacing', 'w:lineRule'],
            ...
            'shadowFill'          => [self::READ_VALUE, 'w:shd', 'w:fill'],
            'shadowColor'         => [self::READ_VALUE, 'w:shd', 'w:color'],
            'shadowVal'           => [self::READ_VALUE, 'w:shd', 'w:val'],
        ];
    ...
    protected function readFontStyle(XMLReader $xmlReader, \DOMElement $domNode)
    {
        ...
        $rPrs = $xmlReader->getElements('w:rPr', $domNode);
        $styleDefs = [
        ...
        if ($rPrs->length > 1) {
            $styles = [];
            foreach ($rPrs as $rPr) {
                $styles[] = $this->readStyleDefs($xmlReader, $rPr, $styleDefs);
            }
            $mergedStyles = [];
        ...
        return  $this->readStyleDefs($xmlReader, $rPrs->item(0), $styleDefs);
    }
